...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string; format: string }> }) {
  ...
  try {
    ...
    if (format === "md") return new Response(report.markdown, { headers: downloadHeaders("text/markdown; charset=utf-8", `incident-${id}.md`) });
    ...
    return new Response(new Uint8Array(pdf), { headers: downloadHeaders("application/pdf", `incident-${id}.pdf`) });
  } finally {
    await browser?.close();
    if (reserved) activeRenders -= 1;
  }
