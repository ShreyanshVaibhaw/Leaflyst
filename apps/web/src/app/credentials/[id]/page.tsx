...
import { RevocationPanel } from "@/components/revocation-panel";
import { apiGet, formatDate, humanize, type Credential, type ImpactPreview, type TimelineEvent } from "@/lib/api";
...
export default async function CredentialPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  let impact: ImpactPreview;
  try { [credential, events, impact] = await Promise.all([apiGet<Credential>(`/v1/dashboard/credentials/${id}`), apiGet<TimelineEvent[]>(`/v1/replay/credentials/${id}/events`), apiGet<ImpactPreview>(`/v1/revocation/${id}/impact`)]); }
