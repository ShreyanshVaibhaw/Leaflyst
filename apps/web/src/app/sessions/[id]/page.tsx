...
export default async function SessionPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ tab?: string }> }) {
  ...
  return <><PageHeader eyebrow="Forensic replay" title={detail.session.agent_id} description={`Session ${detail.session.session_id} · ${detail.session.event_count} recorded events`} action={<div className="flex flex-wrap gap-2"><a href={`/api/exports/sessions/${encodeURIComponent(id)}/report/md`} className="rounded-lg border border-slate-300 bg-white px-3 py-2 text-xs font-semibold">Report · Markdown</a><a href={`/api/exports/sessions/${encodeURIComponent(id)}/report/pdf`} className="rounded-lg border border-slate-300 bg-white px-3 py-2 text-xs font-semibold">Report · PDF</a><a href="/api/exports/evidence" className="rounded-lg border border-slate-300 bg-white px-3 py-2 text-xs font-semibold">Tenant chain evidence</a><form action={action}><button className="rounded-lg bg-slate-950 px-4 py-2 text-sm font-semibold text-white">Create read-only link</button></form></div>} /><SessionReplay detail={detail} tab={query.tab} /></>;
}
