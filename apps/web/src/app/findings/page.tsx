...
export default async function FindingsPage({ searchParams }: { searchParams: Promise<{ severity?: string; provider?: string; finding_type?: string }> }) {
  ...
  return <>
    ...
    <Panel className="mb-5 flex flex-wrap gap-2 p-4">
      ...
      {["aws", "github", "gcp"].map((provider) => <Link key={provider} href={`/findings?provider=${provider}`} className={`rounded-full px-3 py-1.5 text-xs font-medium ${filters.provider === provider ? "bg-cyan-700 text-white" : "bg-cyan-50 text-cyan-800"}`}>{provider.toUpperCase()}</Link>)}
    </Panel>
